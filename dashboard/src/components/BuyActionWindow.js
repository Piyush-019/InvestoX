...
const BuyActionWindow = ({ stockUID, onClose }) => {
  ...
  const { closeBuyWindow, user } = useContext(GeneralContext);
  ...
  const handleBuyClick = async (e) => {
    ...
    try {
      ...
      setError(error.response?.data?.message || "Failed to place buy order. Please try again.");
    }
  };
